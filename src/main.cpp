...
uint256 hashGenesisBlock("0x75418479276eaedd9e5da9a34cca8574e0b0380035b2ced3616b7a67bb5824e3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // OctoCoin: starting difficulty is 1 / 2^12
...
{
    // OctoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	/*
		...
		* During the first 6 months, ~31 million OctoCoins will be minted:
			...
			* 0.8% Premine for promotions, marketing, rewards, and dev team compensation.
		  
			* For the first 8 days, there are 888 OctoCoins in each block (Launch Promo + Base Distribution)
		
			* For the following 88 days, there are 88 OctoCoins in each block with a 1/8 chance of hitting a bonus block containing 888 OctoCoins (~123 bonus blocks/day.)

			* During the next 88 days, each block contains 88 OctoCoins.
		...
		* Through the next few decades, beginning at block 180,656, the remaining OctoCoins (~58 million) will be minted.
			...
			* Block reward is halved every year.
	...
	*/
	...
	{
		nSubsidy >>= ((nHeight - BlockCountA) / (BlockCountB)); // Subsidy is cut in half every 358364 blocks (1 year)
	}
	
    return nSubsidy + nFees;
}

int nTargetTimespan = 888; // 888 seconds (~15 minutes/10 blocks)
static const int nTargetSpacing = 88; // 88 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if((nHeight % nInterval != 0) || (fTestNet)) {

        // Special difficulty rule for testnet:
        if (fTestNet)
        {
            // If the new block's timestamp is more than 2*
    ...
    // The 1st re-target after genesis
    ...
    int nActualTimespanMax = nTargetTimespan*1088/1000;
    int nActualTimespanMin = nTargetTimespan*912/1000;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // OctoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6defc6786c15f01f35bec36682f8f19eca69f2faed4671bba353aab32ed94be0");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1394267888;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1101892;

        if (fTestNet)
        {
            block.nTime    = 1394183844;
            block.nNonce   = 397836;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x91c3cf5f1dabaacabac7f91ff1ba573d233a7826fc51b6d9a308166b6ca9e530"));	
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // OctoCoin: increase each by adding 2 to bitcoin's value.
